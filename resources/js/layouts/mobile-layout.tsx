...
import { BookOpen, ClipboardList, History, Home, ImageIcon, Settings } from 'lucide-react';
...
function getActiveTab(path: string): string {
    ...
    if (path.startsWith('/questions') || path.startsWith('/tickets')) return 'tickets';
    if (path.startsWith('/signs')) return 'signs';
    return 'home';
}

export default function MobileLayout({ children, title = 'მართვის მოწმობა' }: MobileLayoutProps) {
    const { url } = usePage();
    ...
    return (
        <div className="fixed inset-0 flex flex-col overflow-hidden bg-background">
            ...
            <header className="flex-none flex items-center justify-between border-b bg-background/95 pt-[var(--inset-top)] pl-[calc(var(--inset-left)+1rem)] pr-[calc(var(--inset-right)+1rem)] h-[calc(var(--inset-top)+3.5rem)] backdrop-blur supports-[backdrop-filter]:bg-background/60">
            ...
            <main className="min-h-0 flex-1 overflow-y-auto overscroll-contain touch-pan-y [-webkit-overflow-scrolling:touch]">
                {children}
            </main>
