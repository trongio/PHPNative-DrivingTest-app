...
function SheetContent({
  ...
  // For right-side sheets, close button on left; for left-side sheets, close button on right
  const closeButtonPosition = side === "left" ? "right-4" : "left-4";
  const closeButtonRotation = side === "left" ? "rotate-180" : "";
      ...
      >
        {!hideClose && (side === "left" || side === "right") && (
          <SheetPrimitive.Close
            className={cn(
              "absolute flex h-8 w-8 items-center justify-center rounded-full transition-colors hover:bg-muted focus:outline-none",
              closeButtonPosition
            )}
            style={{ top: 'calc(var(--inset-top, 0px) + 1.25rem)' }}
          >
            <ArrowLeft className={cn("h-5 w-5", closeButtonRotation)} />
